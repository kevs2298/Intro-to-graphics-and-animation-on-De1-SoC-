 ...
 * can be written to the driver through this file: "clear", "sync", "pixel x,y color", and
...
int pixel_buffer, backbuffer;						// current location of pixel buffer memory
...
static int __init start_video(void)
{
	...
	/* 3. First store the physical address of the back buffer, because we can't directly write 
	to the register that holds the address of the front buffer */
	backbuffer = *(pixel_ctrl_ptr + 0x1); 

	// 4. Get virtual addresses for the pixel buffer
	pixel_buffer = (int) ioremap_nocache (0xC8000000, 0x0003FFFF);
	if (pixel_buffer == 0)
		printk (KERN_ERR "Error: ioremap_nocache returned NULL\n");

	// 5. Now, swap the front and back buffers, to initialize the front pixel buffer location
	wait_for_vsync(pixel_ctrl_ptr);

	/* 6. Initialize the location of the back pixel buffer in the pixel buffer controller.
	 * This is the same address that we used for the front pixel buffer, which means that there 
	 * is only one pixel buffer used in this program. */
	backbuffer =  *(pixel_ctrl_ptr + 0x1); 
...
void wait_for_vsync(volatile int * pixel_ctrl_ptr)
{
	...
	*pixel_ctrl_ptr = 1;											// start the synchronization process
	...
	}// wait until the S bit turns to 0
}
