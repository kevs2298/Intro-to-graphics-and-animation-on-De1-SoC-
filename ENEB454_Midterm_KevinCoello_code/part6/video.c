 ...
 * "line x1,y1 x2,y2 color", "box x1,y1 x2,y2 color", "text x,y string", and "erase".
...
void get_screen_specs(volatile int *); // get size of pixel buffer
void clear_screen(void);			   // clear pixel bufer
void wait_for_vsync(volatile int *);
void get_char_specs(volatile int *); // get size of character buffer
void erase_chars(void);				 // clear character buffer
...
#define VIDEO_BYTES 14 // number of bytes to return when read (includes a \n)
#define VIDEO_SIZE 128 // max number of bytes accepted for a write
...
volatile int *char_ctrl_ptr;	// virtual address of character buffer controller
int back_buffer;				// current location of pixel back buffer
void *FPGA_ONCHIP_virtual;		// virtual addresses for the FPGA on-chip memory
void *char_buffer;				// virtual addresses for the character buffer
void *SDRAM_virtual;			// virtual addresses for the SDRAM memory
int resolution_x, resolution_y; // VGA screen size
int char_x, char_y;				// character screen size
// int pixel_buffer;
...
static int __init start_video(void)
{
	...
	back_buffer = *(pixel_ctrl_ptr + 1); // set current back buffer address
	...
	char_buffer = ioremap_nocache(0xC9000000, 0x00001FFF);
	if (char_buffer == 0)
		printk(KERN_ERR "Error: ioremap_nocache returned NULL\n");
	get_char_specs(char_ctrl_ptr);
	erase_chars();
	return 0;
}
...
//========= ENEB454 - Add your code here ==========
/* Function to find the character buffer specifications */
void get_char_specs(volatile int *char_ctrl_ptr)
{
	int resolution_reg;

	resolution_reg = *(char_ctrl_ptr + 0x2);	  // TODO
	char_x = (resolution_reg & 0x0000FFFF);		  // TODO
	char_y = (resolution_reg & 0xFFFF0000) >> 16; // TODO
}
...
static ssize_t device_read(struct file *filp, char *buffer, size_t length, loff_t *offset)
{
	...
	if (bytes)
	{
		sprintf(themessage, "%03d %03d %02d %02d\n", resolution_x, resolution_y, char_x, char_y);
...
static ssize_t device_write(struct file *filp, const char *buffer, size_t length, loff_t *offset)
{
	...
	sscanf(video_string, "%s", command);
	if (strcmp(command, "--") == 0)
	{
		printk(KERN_ERR "Intel FPGA sample device driver\n");
		printk(KERN_ERR "Returns: columns rows textcolumns textrows\n");
	...
	else if (strcmp(command, "pixel") == 0)
	{
		if (sscanf(video_string, "%s %d,%d %X", command, &x1, &y1, &color) != 4)
			printk(KERN_ERR "/dev/%s: unable to parse user input: %s\n", DEV_NAME, video_string);
		else if ((x1 < 0) || (x1 > resolution_x - 1) || (y1 < 0) || (y1 > resolution_y - 1))
			printk(KERN_ERR "/dev/%s: bad coordinates: %s %d,%d\n", DEV_NAME, command, x1, y1);
		else
			//========= ENEB454 - Add your code here ==========
			// TODO
	...
	{
		if (sscanf(video_string, "%s %d,%d", command, &x1, &y1) != 3)
		...
		else
		{
			...
			j = 0;
			num_spaces = 0;
			// find the start of the string that is to be printed
			while ((num_spaces) != 2 && (i < strlen(video_string)))
				if (video_string[i++] == ' ')
					++num_spaces;
			if (num_spaces != 2)
				printk(KERN_ERR "/dev/%s: can't parse user input: %s\n", DEV_NAME, video_string);
			...
			{
				// i now points to the start of the string that is to be printed
				// copy the string to be printed into msg
				while ((video_string[i] != '\0') && (video_string[i] != '\n'))
					msg[j++] = video_string[i++];
				msg[j] = '\0';
				if (((x1 + strlen(msg)) > char_x) || (x1 < 0) || (y1 >= char_y) | (y1 < 0))
					printk(KERN_ERR "/dev/%s: bad coordinates: %s %d,%d %s\n", DEV_NAME,
						   command, x1, y1, msg);
				else
				{
					//========= ENEB454 - Add your code here ==========
